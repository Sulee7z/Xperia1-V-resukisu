...
#ifdef CONFIG_KSU_BOOTCONFIG_SPOOF
/* independent bootconfig spoofing (no SUSFS): /proc/bootconfig shows the
 * stock-like buffer below; kernel keeps using the real boot config. */
static char spoof_boot_config[2048] = CONFIG_KSU_BOOTCONFIG_SPOOF_STRING;
#endif
...
#endif
#ifdef CONFIG_KSU_BOOTCONFIG_SPOOF
	if (spoof_boot_config[0]) {
		/* the Kconfig string uses literal \n sequences; emit real newlines */
		const char *p = spoof_boot_config;
		while (*p) {
			if (p[0] == '\\' && p[1] == 'n') {
				seq_putc(m, '\n');
				p += 2;
